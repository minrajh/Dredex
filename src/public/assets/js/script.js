...
const btn__close = document.querySelectorAll('.btn__close'); // Добавил обработчик на кнопку Ок
...
/* Непонятно для чего скрипт */
function Dredex (dataObj) {
    let signals = {}
    observeData(dataObj)
return {
    data: dataObj,
...
/* Слайдер на главной странице сайта НАЧАЛО */
var slideIndex = 1;
...
/* Слайдер на главной странице сайта КОНЕЦ */
...
function observeData (obj) {
    ...
    // Мы можем безопасно анализировать DOM в поисках привязок после преобразования dataObject.
    parseDOM(document.body, obj)
}
  
function syncNode (node, observable, property) {
    node.textContent = observable[property]
    // Мы удаляем `Dredex.`, поскольку он теперь доступен для нас в нашей области.
...
function makeComputed (obj, key, computeFunc) {
    let cache = null
    // Создайте локальный список deps, аналогичный makeReactive deps
    let deps = []

    observe(key, () => {
    cache = null

    // Очистить и уведомить действительные депс
...
Object.defineProperty(obj, key, {
    get () {
    // Если оценивается во время оценки
    // другое вычисляемое свойство
    if (Dep.target) {
        // Создание отношения зависимости
        // между этими двумя вычисляемыми свойствами
        Dep.depend(deps, key)
    }
    // Нормализовать Dep.target обратно на себя
    // Это позволяет построить дерево зависимостей
    // вместо плоской конструкции
    Dep.target = key

    if (!cache) {
        // Очистить список зависимостей, чтобы получить новый
        Dep.subs[key] = []
        cache = computeFunc.call(obj)
    }

    // Очищаем целевой контекст
    Dep.target = null
    return cache
    },
    set () {
...
/* Выравнивания высоты или ширины колонок. */
...
// Простая функция по нажатию на кнопку
function alerted(){
...
window.onload = function () {
    ...
    a.onclick = function() {
        ...
        if (this.innerHTML=='Вкл') this.innerHTML = 'Выкл';
        else this.innerHTML = 'Вкл';
        //предотвращаем переход по ссылке href
        return false;
    }
}

/* функция возвращает cookie с именем dredex, если есть, если нет, то undefined  */   
function getCookie(dredex) {
...
cookiewin = document.getElementById('cookie-modal');    
...
/* Удаление отступов при нумерации */
